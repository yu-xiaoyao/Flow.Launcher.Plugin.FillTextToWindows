...
- **数据**：一个输入框一段，点「+ 添加数据」加一个，点每行后面的「删除」去掉一个；
  序号就是执行顺序。内容**可以留空**，留空的这一段不粘贴，只有这一段的延迟和按键照常执行，
  用来「空过」某个输入框（不填，但要 `Tab` 过去）。内容前后带的空格会原样保留，不会自动去掉
- **自定义按键**：不勾选时这三个按键走插件的全局配置；勾上后这条记录用自己的一套，
  勾上的一瞬间会先用当前全局值填好，所以只想改「切换键」的话改那一个就行
- **开始前等待 / 粘贴后等待 / 按键间隔 / 还原剪贴板**：这四项**不用勾上面的开关也能改**，
  而且是逐项生效的——留空就跟着全局设置走，填了才用这条记录的值。
  所以「只想让这条记录等久一点」就只在「粘贴后等待」里填个数，另外几项继续跟着设置面板变。
  左边会有一行灰字告诉你留空时会跟到哪个值；
  「还原剪贴板」是勾选框，点一下切换：不还原 → 还原 → 跟全局走（半选）。
- **数据行配置模式**：再往下一层，每一段配自己的延迟和按键。勾上后每段数据下面会多出一行小框。
  两个延迟每一段都有，按键**只显示这一段真正用得上的**：
  - 第 1 段：`按键前延迟` `按键后延迟` `开始前` `粘贴后`；
  - 中间几段：`按键前延迟` `按键后延迟` `粘贴后`；
  - 最后一段：`按键前延迟` `按键后延迟` `最后之后`（只有一段数据时是前两个 + `开始前` + `最后之后`）。

  各个框的意思：
  - `按键前延迟`：**这一段的按键发出去之前先等这么久，每一段各等各的**（留空就是不等）。
    和主表的「开始前等待」不是一回事——那个整批只等一次，在最开始发任何按键之前。
    某个字段反应慢、粘贴太快会丢的时候，就单独给它加一点；
  - `按键后延迟`：这一段粘贴（`Ctrl+V`）之后、发后面的按键之前等多久，给目标程序留出处理这次粘贴的时间。
    **留空或者填 0 都是用主表的「粘贴后等待」**，只有填了大于 0 的数才盖掉主表那份；
  - `开始前`：这一段粘贴之前发的按键，接在主表的「开始前按键」后面。**只有第 1 段的会执行**——
    开始前按键整批只在第一个粘贴之前发一次，所以这个框也只画在第 1 段上；
  - `粘贴后`：这一段粘贴之后发的按键，**填了就顶掉主表的「切换输入框」**，留空则继续用主表的。
    最后一段用的不是它，所以最后一段不显示这个框；
  - `最后之后`：只有最后一段用得上，排在主表的「最后一段之后按键」前面。

  这些都和主表（自定义按键，或没勾时的全局配置）是**叠加**关系，不是替换。
  想后面几段用 `Down`、其余用 `Tab`，或者最后一段填完直接 `Enter`，用这个模式就不用一条记录拆成好几条。
  没开这个模式时，行上的延迟和按键原样存着但不生效，关掉再打开不用重新录。
...
1. 按回车，Flow Launcher 隐藏窗口，焦点回到你刚才在用的程序，先等「开始前等待」，插件这时候才开始发按键；
2. 按主表的「开始前按键」，接着按第 1 段自己的「开始前按键」（开了数据行配置模式才有）；
3. 每段文本：先等这一段的「按键前延迟」（开了数据行配置模式才有）→ 内容为空就跳过这一步，
   否则写入剪贴板 → 等主表的「粘贴后等待」→ `Ctrl+V`
   → 等这一段的「按键后延迟」（没单独设就用主表的「粘贴后等待」）
   → 按这一段的「粘贴后按键」（没填就用主表的「切换输入框的按键」）→ 下一段；
...
| 开始前等待（毫秒） | 0 | 一开始先等这么久再发按键，给目标程序一点拿到焦点的时间 |
